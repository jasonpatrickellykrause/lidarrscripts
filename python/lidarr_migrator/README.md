...
Python scripts for exporting and importing artists and albums between Lidarr instances. Useful for migrating a music library to a new server or for backup/restore operations.
...
Both scripts also accept the API key via the `LIDARR_API_KEY` environment variable instead of `--api-key`, which keeps it out of shell history.
...
The import script checks this against the target instance before starting and exits with an error if the path isn't registered there.
...
| `--api-key` | Yes* | Lidarr API key (*or set `LIDARR_API_KEY`) |
...
| `--api-key` | Yes* | Target Lidarr API key (*or set `LIDARR_API_KEY`) |
